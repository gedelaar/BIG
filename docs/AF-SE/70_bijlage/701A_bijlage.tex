...
\begin{obs}[{rq1-13:17-10}:
    ...
    But it still didn't work]\label{obs:rq1-13:17-10}
...
\begin{obs}[{rq1-96:30-12:} Skill in scripting within Ampersand is quickly lost if you don't do this frequently.]\label{obs:rq1-96:30-12}
...
\begin{obs}[rq4-1 Ampersand cannot calculate.
    But since {Ampersand} is static, process data can be monitored in other ways.]\label{obs:rq4-1}
...
\begin{obs}[{rq1-48:27-10:} The concept current date is solved very complicated.
...
\begin{obs}[{rq1-46:24-10:} There is no find able relationship between the {relation} and the concept in the script.]\label{obs:rq1-46:24-10}
...
\begin{obs}[{rq1-43:23-10:} The order of the data in the Conceptual analysis is a bit strange.
    ...
    \textbf{obs}: The layout of the Conceptual design doesn't seem quite logical and is therefore confusing.
\end{obs}
...
\begin{obs}[{rq1-41:19-10:} The "wettenbank" website contains a persistent hyperlink, which can be used in the documentation as reference.]\label{obs:rq1-41:19-10}
    \textbf{obs}: References to persistent links can be included.
...
\begin{obs}[{rq1-82:20-11:} include don't always seem necessary on compilation.
    ...
    \textbf{obs}: Includes are useful and necessary, but it's not always clear how to use them.
\end{obs}
...
\begin{obs}[{rq1-53:2-11:} The {crud} (\acrlong{crud}) and \acrshort{crud} in the interface don't always work as it should be.
...
\begin{obs}[{rq1-58:8-11:} Per interface max one {multiplicity}, otherwise you won't get data stored.]\label{obs:rq1-58:8-11}
...
\begin{obs}[{rq1-5:30-10:} The browser is holding data from the interface and periodically the cache needs to be cleared for customization to work.]\label{obs:rq1-5:30-10}
    \textbf{obs}: It looks like the changes made to the script don't affect operation.
...
\begin{obs}[{rq1-24:12-9} {XML} download from wetBig seems like a logical step for the analysis and processing, but it is too complex.
    ...
    The thought that comes to mind here is why SDU doesn't directly annotate the concepts and relationships.]\label{obs:rq1-24:12-9}
...
\begin{obs}[{rq1-57-2:7-11:} Parts like next big number or now() and today() are better solved in a dev language, like {php}.]\label{obs:rq1-57-2:7-11}
    \textbf{obs}: A development language like php because it's in the Ampersand software stack.
\end{obs}
...
\begin{obs}[rq3-9:19-9: The structure of the {register}'s is the same, registers are also called registrations]\label{obs:rq3-9:19-9}
...
\begin{obs}[{rq1-17} Applying a {rule}\textbf{s} takes a lot of patience and practice.
...
\begin{obs}[rq1-61:9-11: There should be a check on the draft date of birth({rule}), so that someone must be at least 18.
    ...
    \textbf{obs}: You don't have to think of anything yourself.
...
\begin{obs}[{rq1-22} The tool VSC also doesn't have a generic search option across the adls.]\label{obs:rq1-22}
