...
\sbbs{1}{Architectural fit}\label{s:4_1_architectural_fit}
\POstart{%1
...
Ampersand its design must fit the architecture of the \acrshort{cibg} organization where it is used (\oref{obs:rq1-63:10-11}).
We see that the design that follows from the Ampersand method overlaps with the existing \acrlong{rk} (\iref{int:I-1.3}).
Mapping is needed to align Ampersand with \acrlong{rk} (\iref{int:I-2.7}, \oref{obs:rq1-89:7-12}, \iref{int:I-1.12}, \iref {int:I-2.6}).
...
In particular, the generic part of the design is related to \acrlong{rk} (\iref{int:I-2.1}).
Ampersand cannot calculate out-of-the-box.
\sbbs{2}{Conceptual analysis}\label{s:4_2_conceptual_analysis}
\POstart{%2
...
\sbbs{3}{Lifecycle law}\label{s:4_3_lifecycle_law}
\POstart{%3
...
\sbbs{4}{Register unbundling}\label{s:4_4_register_unbundling}
\POstart{%4
...
\sbbs{5}{Test scenario}\label{s:4_5_test_scenario}
\POstart{%5
...
\sbbs{6}{\acrfull{brp}}\label{s:4_6_brp}
\POstart{%6
...
In the context of the design you want to have observations made explicit (\iref{int:I-4.7}, \oref{obs:rq2-17:10-11}).
Matters such as address management, country management and address formatting are not included in the law (\oref{obs:rq3-10:12-10}), but these items are part of the design.
...
\sbbs{7}{Total design}\label{s:4_7_total_design}
\POstart{%7
...
\sbbs{8}{User experience}\label{s:4_8_user_experience}
\POstart{%8
...
Ampersand its design is such that a new design is always delivered (\iref{int:I-4.8}).
\sbbs{9}{Excluded}\label{s:4_9_excluded}
\POstart{%9
