...
\sbbs{1}{Setup}\label{s:1_1_setup}
\POstart{%1
...
\sbbs{2}{Script creation}\label{s:1_2_script_creation}
\POstart{%2
    \POS{obs:rq1-60:9-11}
...
\sbbs{3}{Source handling}\label{s:1_3_source_handling}
\POstart{%3
    \POS{obs:rq1-2}
    \POS{obs:rq1-25:12-9}
    \POS{obs:rq1-31:14-9}
...
To maintain the overview at work, resources are needed to maintain this overview (ob.\ref{obs:rq1-25:12-9}, \iref{int:I-3.7}).
...
\sbbs{4}{Script overview}\label{s:1_4_script_overview}
\POstart{%4
...
\sbbs{5}{Script metadata}\label{s:1_5_script_metadata}
\POstart{%5
    \POS{obs:rq3-7}
    \PIS{int:I-3.2}}

Adding the information to the Ampersand concepts is not very straightforward (obs.\ref{obs:rq3-7}).
...
There is a connection between the description in the meta information and the \acrlong{ca}, because the \acrlong{ca} must be clearly readable (\iref{int:I-3.2}).
\sbbs{6}{Deviation}\label{s:1_6_deviation}
\POstart{%6
...
\sbbs{7}{Architecture and registerkern}\label{s:1_7_architecture_and_registerkern}
\POstart{%7
...
\sbbs{9}{Model maintenance}\label{s:1_9_model_maintenance}
\POstart{%9
    \POS{obs:rq1-63:10-11}
    \POS{obs:rq4-7}
    \PIS{int:I-4.8}
...
There is no possibility to implement a modified model and simply transfer the data to the new model, error free specifications (\iref{int:I-2.5}).
Ampersand its new model stands alone and has no physical relationship with the originally implemented model (\iref{int:I-4.8}).
...
\sbbs{10}{Ampersand design method}\label{s:1_10_ampersand_design_method}
\POstart{%10
    \POS{obs:rq2-18:16-11}
    \PIS{int:I-4.6}}

A project always has to be designed.
One of the interviewees commented that it does not matter much in which tool that happens (\iref{int:I-4.6}).
...
\sbbs{11}{Law effective}\label{s:1_11_law_effective}
\POstart{%11
    \PIS{int:I-1.2}
    \PIS{int:I-1.6}}
...
\sbbs{12}{Excluded}\label{s:1_12_exclude_1}
\POstart{%12
    \PIS{int:I-2.11}
    \PIS{int:I-4.11}
    \PIS{int:I-4.12}
